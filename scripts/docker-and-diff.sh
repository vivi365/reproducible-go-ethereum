...
rm ~/reproducible-go-ethereum/bin/geth-local && rm ~/reproducible-go-ethereum/bin/geth-reference
...
echo "\nCopying binaries..."
docker cp -q "$CONTAINER_ID":/bin/geth-local "$OUTPUT_DIR"/geth-local
docker cp -q "$CONTAINER_ID":/bin/geth-reference "$OUTPUT_DIR"/geth-reference
...
md5_local=$(md5 "$OUTPUT_DIR"/geth-local | awk '{print $NF}')

echo "\nReference build has hash $md5_reference\nReproducing build has hash $md5_local"

if [ "$md5_local" != "$md5_reference" ]; then
    echo "\nBinaries mismatch. Running diffoscope..."
    cd "$OUTPUT_DIR" || exit 1
    docker run --rm -t -w "$(pwd)" -v "$(pwd)":"$(pwd)":ro registry.salsa.debian.org/reproducible-builds/diffoscope geth-reference geth-local
else
    echo "Binaries match."
fi
